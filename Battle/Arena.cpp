...
void Arena::Fight(int input) 
{
	...
	{
		int ADamage = A->AssignAttackDamage(50/input);
		int BDamage = B->AssignAttackDamage(50/input);
...
{
	int any = 0;
	std::cout << "Type '0 or 1++' to start again! or Type -1 to exit \n";
	std::cin >> any;
	if (any > -1) 
	{
		A->SetHp(100);
